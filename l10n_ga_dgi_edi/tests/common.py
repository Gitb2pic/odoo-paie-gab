...
class GaDeclarationCase(GaPayrollCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.company.write({'l10n_ga_nif': 'NIF-TEST', 'l10n_ga_cnss_number': 'CNSS-EMP'})
    ...
    def _declaration_type(cls, code, auto_create=False, **values):
        boxes = [
            ...
            ('B_IRPP', 'IRPP', 'amount', {'source_codes': 'GA_IRPP, GA_IRPP_REGUL', 'negate': True, 'is_total': True}),
            ('B_TCS', 'TCS', 'amount', {'source_codes': 'GA_TCS', 'negate': True, 'is_total': True, 'cell_ref': 'C11'}),
            ('B_FNH', 'FNH', 'amount', {'source_codes': 'GA_FNH', 'is_total': True, 'cell_ref': 'C12'}),
            ('B_CNSS', 'CNSS salariale', 'amount', {'source_codes': 'GA_CNSS_SAL', 'negate': True}),
        ]
